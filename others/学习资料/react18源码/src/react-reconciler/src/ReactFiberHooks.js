...
function updateEffectImpl(fiberFlags, hookFlags, create, deps) {
  ...
  if (currentHook !== null) {
    ...
    if (nextDeps !== null) {
      ...
      // api beginWork areHookInputsEqual 依赖判断
      if (areHookInputsEqual(nextDeps, prevDeps)) {
        // 这个effect是多余的，没有HookHasEffect，所以不执行，
        // 只是为了让链表完整
...
function mountEffectImpl(fiberFlags, hookFlags, create, deps) {
  ...
  currentlyRenderingFiber.flags |= fiberFlags;
  // effect有两种tag
  // 1.HookHasEffect 要不要执行effect
  // 2.是passive还是layout的effect
...
function pushEffect(tag, create, destroy, deps) {
  // function A() {
  // useEffect <= fiber.memoizedStated     |<==== fiber.updateQueue
...
// api hook dispatchSetState
// 就是 setState(action) 根据传入的action,创建一个update，插入queue.pending中
// 调度更新
// 优化：挂载先计算一下，如果值和hook.memoizedState相同，这个update不插入队列直接丢掉
function dispatchSetState(fiber, queue, action) {
  ...
  // 不懂 只有第一次挂载或者第一个更新都能进行此项优化
  // 优化： 为了减少一次update，减少渲染次数。如果一个click事件里面只有有一个setState，并且eagerState和lastRenderedState一样的话，如果连续点击update永远会被丢掉。点多少次，就减少多少次渲染
  // 立即计算一次state,如果和lastRenderedState，这个update直接丢掉
...
/**
 * // api hook updateWorkInProgressHook 更新hook
 */
...
function updateReducer(reducer) {
  ...
  if (baseQueue !== null) {
    ...
    // 不懂   if (fiber.lanes === NoLanes && (alternate === null || alternate.lanes == NoLanes)) {
...
function mountReducer(reducer, initialArg) {
  ...
  hook.queue = queue;
  // api hook dispatchReducerAction
  // 初次挂载的时候执行mountReducer，
  // 点击按钮，触发事件，调用dispatchReducerAction,里面调用scheduleUpdateOnFiber,重新创建fiber树,触发 updateReducer更新数据
...
/**
 * // api hook mountWorkInProgressHook
...
export function renderWithHooks(current, workInProgress, Component, props, nextRenderLanes) {
  ...
  // Component = function AAA() {
  // useState();
  // useEffect();
  //}
  // api beginWork/Component(props)函数组件本体, 函数组件不管如何都会重新执行，但是dom可能不会重新挂载
  // 所有函数都会重新执行，但是性能为什么不会受影响。因为所有的耗时逻辑都放在了useEffect里面当异步执行了。
