...
export default function useTrigger({ content, disabled, trigger, visible, onVisibleChange, triggerRef, delay }) {
  ...
  const  getTriggerNode = React.useCallback((children: TNode) => {
    const triggerNode =
    React.isValidElement(children) && !isFragment(children) ? children : <span className="t-trigger">{children}</span>;
